...
public class WeightedDirectedGraph {
  private final Map<String, Map<String, Integer>>
      graph; // 图是通过list实现的，每个节点用一个map表示，key是邻接节点，value是权重
  // 用于建立节点名称和邻接矩阵编号对应关系
  private final Map<String, Integer> graphnode;
  public int[][] adjTable;
  public int[][] templateTable;
  ...
  public int getNodeIndex(String node) { // 返回节点在邻接矩阵中的索引
    return graphnode.getOrDefault(node, -1);
  }
  ...
  public void gettempadjTable() {
    int size = graph.size();
    templateTable = new int[size][size];
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        templateTable[i][j] = 0;
      }
  ...
  public int istempadjTable(int i, int j) {
    return templateTable[i][j] == 1 ? 1 : 0;
  }
  ...
  public String shortestPath(String start, String end) {
    ...
    if (flag == 3) {
      ...
      for (int i = 0; i < getSize(); i++) {
        ...
        for (int j = 0; j < getSize(); j++) {
          // System.out.println(graph.getNodeName(minIndex) + "->" + graph.getNodeName(j) + ":" +
          // graph.getEdgeWeight(graph.getNodeName(minIndex), graph.getNodeName(j)));
      ...
      if (dist[getNodeIndex(end)] == Integer.MAX_VALUE) {
        ...
        System.out.printf(
            "The shortest path from \"%s\" to \"%s\" is: %d\n",
